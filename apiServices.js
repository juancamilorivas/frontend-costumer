...
export const fetchPostsOnSnapshot = (postsPerLoad, onData, onError) => {
  const q = query(
    collection(db, "blogPosts2"),
...
export const fetchMorePostsOnSnapshot = (postsPerLoad, lastVisible, onData, onError) => {
  const q = query(
    collection(db, "blogPosts2"),
    orderBy("createdAt", "asc"),
    limit(postsPerLoad),
